...
class ClipsService {
  ...
  async createClipComment(clipId, formData, userInfo) {
    ...
    await clipComment.populate("creator", "name picture");
    return clipComment;
  }
  async getCommentsByClipId(clipId) {
    const clipComments = await dbContext.Comments.find({ clipId }).populate(
      "creator",
      "name picture"
    );
    return clipComments;
  ...
  async getDislikes(query = {}) {
    const dislikes = await dbContext.dislikes
      .find(query)
      .populate("clip")
      .populate("dislike", "name picture");
  }
  ...
  async getClipById(id) {
    ...
    const clip = await dbContext.Clips.findById(id)
